...
class Orchestrator:
    def __init__(self, platform: Platform, run_script: str):
        ...
        self.output_file = open(
            "output_results_" + run_script.split("/").pop().split(".")[0], "w+"
        )
    ...
    def perform_experiment(self, measurements_interval: float = 1):
        ...
        telemetry_thread = Telemetry(
            "telemetry_" + self.run_script.split("/").pop().split(".")[0]
        )
        telemetry_thread.write_new_current_frequencies(
            self.variator.current_frequencies, self.platform
        )
        ...
        if self.platform != Platform.GEFORCE:
            ...
            while valid_frequency:
                ...
                process_output = process.stdout + "\n" + process.stderr
                self.output_file.write(process_output)
                valid_frequency = valid_frequency = self.variator.variate_frequency_up(
                    "memory"
                )
                telemetry_thread.write_new_current_frequencies(
                    self.variator.current_frequencies, self.platform
